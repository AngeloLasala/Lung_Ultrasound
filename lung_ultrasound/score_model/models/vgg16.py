"""
Create VGG16 model for LUS multi instance classification compatible with CAM computation
"""
...
class VGG16LUSCAM(nn.Module):
    """
    VGG16 model for Lung Ultrasound (LUS) multi-instance classification
    with support for Class Activation Maps (CAM) computation.
    
    This model uses a pre-trained VGG16 backbone and modifies it for
    multi-instance learning scenarios common in medical imaging.
    """
    
    def __init__(self, num_classes=2, pretrained=True,  backbone='vgg16',
                 freeze_backbone=False, pooling='avg', batch_norm=True):
        """
        Initialize VGG16 model for LUS classification.
        ...
        Args:
            ...
            batch_norm (bool): Whether to use VGG16 with batch normalization
        """
        super(VGG16LUSCAM, self).__init__()
        
        # Load appropriate VGG16 backbone
        if backbone == 'vgg16':
            self.backbone = models.vgg16(pretrained=pretrained)
        else:
            raise ValueError(f"Unsupported backbone: {backbone}")
        
        # VGG16 feature dimension from last conv layer
        self.feature_dim = 512
        
        # Extract only the convolutional features (remove classifier)
...
if __name__ == "__main__":
    ...
    model = VGG16LUSCAM(num_classes=5, pretrained=True, batch_norm=True)
